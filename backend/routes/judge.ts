...
// Submit team score
router.post("/score", modifyLimiter, logActivity("SUBMIT_TEAM_SCORE"), async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
// Get team score
router.get("/teams/:teamId/score", async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const round = req.query.round ? parseInt(req.query.round as string, 10) : 1;
